...
function makeControlSection(voltageName) {
    let ctrlSection = '';
    ctrlSection += '.control\n';

    console.log(plotType);
    console.log(voltageName);
    ...
    switch (plotType) {
        ...
        case 'DCSweep':
            console.log(voltageName);
            ctrlSection += `dc ${voltageName} ${startVolt} ${endVolt} ${step}\n`;
            break;
        case 'Transient':
...
function makePrintLines(voltProbes, currentProbes, controlSectionSwitch) {
    console.log(voltProbes, currentProbes);
    let printLines = '';
...
async function ModifyNetlist(filePath) {
    try {
        ...
        for (let line of lines) {
            let parts = line.trim().split(/\s+/); // Split line into parts
            if (parts.length > 1) {
                let firstChar = parts[0][0].toUpperCase();
                if (firstChar === 'V' || firstChar === 'I') {
                    VoltName = parts[0];  // Just grab the name
                    break; // Found the source, done
                }
            }
        }

        console.log("Voltage Name:", VoltName);



        //This gives us .control
        //+ simulation type
        let controlSection = makeControlSection(VoltName);
